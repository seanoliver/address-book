...
**Files:**
- Create: `src/lib/csv/import.ts` (+ `.test.ts`), `src/app/dashboard/import/page.tsx`, `src/app/dashboard/import/import-form.tsx`, `src/app/dashboard/import/actions.ts`, `src/lib/db/contact-row.ts` (shared `toRow`), `public/contacts-template.csv`

**Step 1: Failing tests** for `parseContactsCsv(text)`: maps canonical headers (`full_name,partner_name,kids_names,email,birthday,address_line1,address_line2,city,state_region,postal_code,country,notes`), tolerates common aliases case-insensitively with surrounding whitespace (`name`→full_name, `partner`/`spouse`→partner_name, `kids`/`children`→kids_names, `zip`/`zipcode`/`postcode`→postal_code, `state`/`province`/`region`→state_region, `address`/`street`/`address1`/`street_address`→address_line1, `address2`→address_line2, `e-mail`/`email_address`→email, `dob`/`date_of_birth`→birthday, `note`→notes), strips the Excel UTF-8 BOM, ignores unknown columns, returns `{ valid: ContactInput[], errors: { row: number, message: string }[] }` (1-based data row numbers; row 0 = file-level error such as no recognizable headers or empty file), caps at 1000 rows via papaparse `preview` (single "Row limit is 1000" error entry, excess rows never parsed), validates each row with `contactSchema`.

**Step 2–3: Implement with Papaparse (`header: true, skipEmptyLines: true, preview: 1001`), tests green, commit.** The module is isomorphic (no server-only imports) — it runs in the browser for the preview.

**Step 4: Import page**: server page (`requireUser` + book gate) + client `ImportForm` — file input (reject >1 MB before parsing) → parse in browser → dedupe in-file duplicate emails client-side (case-insensitive to mirror citext; first occurrence wins, counted as skipped) → preview table (first 20 ready rows) + full error list + count summary → confirm → server action `importContacts(rows)` which re-validates server-side (`z.array(contactSchema).max(1000)`) and bulk-inserts via `withRls` with `onConflictDoNothing({ target: [bookId, email], where: sql\`email is not null\` })` (the `where` is the partial-index predicate of `contacts_book_email_unique`); rows whose email already exists are skipped, `skipped = attempted − returning().length`. Rows without an email never conflict, so re-importing duplicates them — accepted v1 behavior. Bulk import writes NO `contact_events` audit rows (documented decision: no import-level event in schema; per-row would need an admin loop). Result `{imported, skipped}` banner (role=status). "Download template CSV" link → `public/contacts-template.csv`.

**Step 5: Verified via ad-hoc Playwright + Mailpit script (10-row fixture: 7 clean incl. aliases+BOM+quoted commas, 1 bad email, 1 missing name, 1 in-file duplicate email): preview 7 ready / 2 errors / 1 dupe → import 7, skip 1 → re-import same file imports 0, skips 8 → no-email rows duplicate on re-import → >1 MB file rejected. Commit.**
...
**Step 5: Dashboard UI**: checkbox column + "Request addresses" button (and "Send to all shown", which targets the visible — possibly search-filtered — rows only, passing their ids explicitly). Confirmation dialog shows the sendable count and the skipped-no-email count before sending; the ids are captured when the dialog opens.
...
**As built (deviations):**
- Book lookup lives in `src/lib/queries/public-book.ts` (`getPublicBook`, allowlisted for `dbAdmin` in eslint.config.mjs): selects ONLY `{title, ownerName, enabledFields}` by slug, validates the slug shape (`SLUG_SHAPE`, mirroring the DB CHECK) before any query — malformed slugs 404 with zero DB work (verified: no rate-limit row). Integration-tested (`public-book.test.ts`) including a key-set assertion so a widened select fails a test.
- Page order is slug-shape → rate limit `permalink-view` 60/h per hashed IP (fail closed) → lookup → `notFound()`. Deliberately NO robots noindex — the permalink is meant to be shared; the page is write-only and its DOM carries only title/owner name/enabled flags.
- Action order (rate limits and Turnstile BEFORE Zod): slug shape → `permalink-submit:` hashed-IP 5/h → Turnstile → `permalink-book:${slug}` 100/day → `tokenUpdateSchema` (email optional, full_name required). The IP limit runs first (a client burns its own budget before anything shared) and the book limit runs AFTER the bot gate, so bot-gated junk can never drain a targeted book's 100/day budget. Payload keeps only fields WITH a value — unlike `apply_token_update`, `submit_to_book` stores payload verbatim as a new submission, so present-but-empty keys carry no "clear" meaning and are dropped.
- Success redirects to a static, generic `/b/[slug]/thanks` (no submitted data, no params read, no query params); `submit_to_book` returning false (vanished slug) → `notFound()`. Response and timing identical whether or not the email matched an existing contact (e2e: case-flipped citext match sets `matched_contact_id`, same thanks page, no timing oracle).
- Owner notification (`sendNotification` in resend.ts + `submissionNotificationEmail` in templates.ts): subject "New address submission for {bookTitle}", body links `${APP_URL}/dashboard/review`, ZERO submitted data; owner email via `dbAdmin` books→auth.users join; runs post-response via Next's `after()` so a slow provider never delays the submitter's redirect, and every failure path logs and returns — a notify failure never fails the submission. Dry-run logs `to` + subject only.
- The form is the shared `src/components/recipient-form.tsx` (extracted from Task 13's `token-update-form.tsx`, which it replaces): /u/[token] renders it pre-filled, /b/[slug] renders it blank with the same enabled-field gating (disabled fields absent from the DOM).
...
- Review fix (double-approve race): both approve paths gate on an ATOMIC status flip inside the write transaction (`update … set status='approved' where … and status='pending' returning id`; 0 rows ⇒ a concurrent approval won ⇒ NOT_FOUND, nothing written) — the pending SELECT alone was check-then-act and two concurrent approvals of an email-less submission could both insert. Flip runs after all read-only checks and BEFORE the contact write, so a losing racer writes nothing and a failed insert (duplicate email) rolls the flip back to pending. Guardrailed by `review/approve-race.test.ts` (two barrier-synchronized concurrent transactions → exactly one contact; rollback-restores-pending via the real unique constraint); test cleans up after itself because the pgTAP suite asserts on global submissions counts.
