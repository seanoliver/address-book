...
- Action order mirrors /u/[token] (rate limits and Turnstile BEFORE Zod): slug shape → `permalink-submit:` hashed-IP 5/h → `permalink-book:${slug}` 100/day (IP first so one client burns its own budget before any book's shared budget) → Turnstile → `tokenUpdateSchema` (email optional, full_name required). Payload keeps only fields WITH a value — unlike `apply_token_update`, `submit_to_book` stores payload verbatim as a new submission, so present-but-empty keys carry no "clear" meaning and are dropped.
