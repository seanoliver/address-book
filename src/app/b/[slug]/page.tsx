 ...
 * write-only — the DOM carries nothing beyond title, owner display name, and
 ...
 * counts, no contact data — only {title, owner name, enabled-field flags},
 * all of which the owner chose to publish by sharing the link. No cookies,
...
export default async function PublicBookPage({
  ...
  // 3. Book lookup: {title, owner name, enabled flags} ONLY. Unknown slug →
  ...
  // as /u/[token] and the request email).
  const ownerLabel = book.ownerName.trim() || book.title;
    ...
    <main className="mx-auto w-full max-w-lg flex-1 px-4 py-10">
      <div className="rounded-xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          {book.title} — fill in your details below. Only {ownerLabel} can see
          what you submit.
        </p>
